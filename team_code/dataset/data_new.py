...
import sys
import copy
from .utils import seg_one_hot_key, seg_pre_processing, transform_2d_points, lidar_pre_processing, rgb_specific_seg_pre_processing
...
class CARLA_Data(Dataset):
    ...
    def __getitem__(self, index):
        ...
        for i in range(self.seq_len):
            train_data['front'].append(torch.from_numpy(np.transpose(np.array(Image.open(self.data['front'][index][i])), [2,0,1])))
    ...
    def _load_data(self):
        ...
        for sub_root in tqdm(self.root, file=sys.stdout):
            # TODO: change name
            preload_file = os.path.join(sub_root, 'rg_lidar_diag_pl_' + str(self.seq_len) + '_' + str(self.pred_len) + '_pre_load.npy')
                ...
                for route in routes:
                    ...
                    for seq in range(num_seq):
                        ...
                        for i in range(self.seq_len):
                            ...
                            seq_data['front'].append(os.path.join(route_dir,"rgb_front",filename))

                            # segmentations
                            seq_data['seg_front'].append(os.path.join(route_dir,"seg_front",filename))
                            ...
                            seq_data['x'].append(data['x'])
                            seq_data['y'].append(data['y'])
