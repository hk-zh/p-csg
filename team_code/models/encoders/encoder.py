...
class Encoder(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.image_encoder = ImageEncoder(model=config.image_encoder_model, c_dim=config.c_dim)
        self.lidar_encoder = LidarEncoder(model=config.lidar_encoder_model, c_dim=config.c_dim, in_channels=2)
    ...
    def forward(self, image_inputs, lidar_inputs, measurements):
        ...
        """
        image_inputs /= 255.0 # try to fix this
