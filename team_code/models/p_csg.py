...
class P_CSG(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.front_seg_decoder =  nn.Sequential(
            UnFlatten(),
            nn.ConvTranspose2d(128, 64, kernel_size=(4, 5), stride=2),
            nn.ReLU(),
            nn.ConvTranspose2d(64, 32, kernel_size=(3,4), stride=3),
            nn.ReLU(),
            nn.ConvTranspose2d(32, 16, kernel_size=(3,5), stride=(2,3)),
            nn.ReLU(),
            nn.ConvTranspose2d(16, 8, kernel_size=(3,2), stride=(3,2)),
            nn.ReLU(),
            nn.ConvTranspose2d(8, 4, kernel_size=2, stride=2),
            nn.ReLU(),
            nn.ConvTranspose2d(4, self.front_seg_channels, kernel_size=2, stride=2)
        )
    ...
    def alignment_loss(self, feature1, featrue2, reduction='mean'):
        return F.mse_loss(feature1, featrue2, reduction=reduction)

    def kl_alignment_loss(self, mu1, logvar1, mu2, logvar2, mode=0):
    ...
    def losses(self, ret, gt_waypoints, gt_traffic_light, gt_stop_sign, front_seg, topdown_seg, topdown_seg_ext):
        ...
        # alignment_loss = self.alignment_loss(ret['rgb_sf'], ret['lidar_sf'])
        alignment_loss = self.kl_alignment_loss(ret['mu_front'], ret['logvar_front'], ret['mu_td_s'], ret['logvar_td_s'], mode=2)
        losses = {
            'wp_loss': wp_loss,
    ...
    def forward(self, image_inputs, lidar_inputs, target_point, measurements):
        ...
        # shared_features = (rgb_shared_features + lidar_shared_features) / 2.0
        # fused_features =  torch.cat([shared_features, rgb_unique_features, lidar_unique_features, measurements], dim=1)
        z = self.join(features)
        ...
        if not self.inference:
            ...
            return pred_wp, tl_indicator, ss_indicator
